...
def _find_repo_root(start: Path) -> Path:
    p = start.resolve()
    while p != p.parent:
        if (p / "eva").is_dir() and (p / "examples" / "notebooks" / "interpretability_analysis" / "sae_repro_release").is_dir():
            return p
        p = p.parent
    raise RuntimeError("Cannot locate EVA repo root containing eva/ and examples/notebooks/interpretability_analysis/sae_repro_release")
...
def _import_eva_components():
    try:
        ...
        raise RuntimeError(
            ...
            "'examples/notebooks/interpretability_analysis/sae_repro_release/requirements-sae-training.txt' "
...
def resolve_runtime_paths(cfg: dict, mode: str) -> tuple[str, str, str]:
    ...
    out_root = os.environ.get(
        "SAE_OUTPUT_ROOT",
        str(REPO_ROOT / "examples" / "notebooks" / "interpretability_analysis" / "sae_repro_release" / "outputs" / mode_dir / "checkpoints"),
    )
...
def main() -> None:
    ...
    log_dir = REPO_ROOT / "examples" / "notebooks" / "interpretability_analysis" / "sae_repro_release" / "logs"
