...
def check_repo_layout() -> list[str]:
    ...
    required = [
        REPO_ROOT / "eva",
        REPO_ROOT / "examples" / "notebooks" / "interpretability_analysis" / "sae_repro_release" / "configs",
        REPO_ROOT / "examples" / "notebooks" / "interpretability_analysis" / "sae_repro_release" / "scripts" / "run_training.py",
    ]
    for p in required:
        if not p.exists():
