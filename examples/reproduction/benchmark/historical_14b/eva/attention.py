...
class Attention(nn.Module):
    ...
    def prepare_qkv(
        ...
        # KV cache handling: when use_cache=True and cache object exists, update and get complete key/value
        if use_cache and past_key_value is not None:
            # Convert to [bsz, num_heads, seq, dim] format for cache update
            key_states = key_states.transpose(1, 2)
            val_states = val_states.transpose(1, 2)
            # DynamicCache.update appends new key/value to existing cache and returns complete sequence
            key_states, val_states = past_key_value.update(key_states, val_states, self.layer_idx)
            # Convert back to [bsz, seq, num_heads, dim] format
            key_states = key_states.transpose(1, 2)
            val_states = val_states.transpose(1, 2)
    ...
    def _sdpa_attn(
        ...
        # When using KV cache (q_len=1, k_len>1), causal mask is not needed
        # because we only compute attention from the last token to all historical tokens
        # When q_len == k_len, use is_causal=True to let Flash Attention handle it automatically
        use_causal = (q_len == k_len)

        # Try to use optimized attention kernel, fallback to manual implementation if it fails
        try:
            # First try Flash Attention (fastest)
            ...
            # Flash Attention failed, try without specifying backend (let PyTorch auto-select)
            try:
                attn_output = F.scaled_dot_product_attention(
                    query_states,
                    key_states,
                ...
                # All optimized kernels failed, use manual implementation
                # Manual attention calculation: softmax(Q @ K^T / sqrt(d)) @ V
                attn_weights = torch.matmul(query_states, key_states.transpose(2, 3)) / (self.head_dim ** 0.5)

                # Only apply causal mask when q_len == k_len
                if use_causal:
                    causal_mask_manual = torch.triu(
                        torch.full((q_len, k_len), float('-inf'), device=query_states.device, dtype=query_states.dtype),
                        diagonal=1
                    )
                    attn_weights = attn_weights + causal_mask_manual

                attn_weights = F.softmax(attn_weights, dim=-1, dtype=torch.float32).to(query_states.dtype)
                attn_output = torch.matmul(attn_weights, val_states)
