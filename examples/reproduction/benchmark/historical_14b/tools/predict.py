...
def create_parser() -> argparse.ArgumentParser:
    ...
    calc_group.add_argument(
        "--exclude_special_tokens", action="store_true", default=True,
        help="Exclude 5/3/<eos> token log-likelihood, only compute pure sequence part (default: True)"
    )
    calc_group.add_argument(
    ...
    calc_group.add_argument(
        ...
        help="Only compute log-likelihood on pure sequence tokens, excluding prefix/tag, 5, 3, <eos>"
    )
    calc_group.add_argument(
...
def run_scoring(args):
    ...
    exclude_special = getattr(args, 'exclude_special_tokens', True)  # Default to True
    length_norm = getattr(args, 'length_normalize', False)
    seq_only = getattr(args, 'sequence_only', False)
    ...
    if not args.quiet:
        print(f"\nStarting scoring (mode={args.mode}, normalize={args.normalize}, "
              ...
              f"sequence_only={seq_only}, batch_size={args.batch_size})...")
...
def run_batch_scoring(args):
    ...
    default_exclude_special = batch_config.defaults.get('exclude_special_tokens', True)  # Default to True
