#!/usr/bin/env python3
"""
Sequence Scoring Worker Script (Modified for Benchmark)

This script calculates the log-likelihood of RNA sequences.
Automatically adds 5'/3' direction markers and EXCLUDES them from scoring.

Usage:
    python score_worker_exclude_direction.py <sequences_json> <checkpoint_path> <device> <output_json> [reduce_method] [batch_size]
...
# Use absolute path that works in both host and container
TOOLS_DIR = Path('/rna-multiverse/z_rnagym_70/RNAVerse/tools')
    ...
    for batch_idx in range(num_batches):
        ...
        for i in range(len(batch_sequences)):
            ...
            for j in range(seq_len - 1):
                ...
                # Exclude special tokens (5, 3, <eos>)
...
def main():
    ...
    if len(sys.argv) < 5:
        print(json.dumps({
            'success': False,
            'error': 'Usage: python score_worker_exclude_direction.py <sequences_json> <checkpoint_path> <device> <output_json> [reduce_method] [batch_size]'
        }))
        sys.exit(1)
    ...
    try:
        ...
        print(f"[INFO] Will EXCLUDE 5'/3'/<eos> tokens from log-likelihood calculation", file=sys.stderr)
        ...
        for batch_idx in range(num_batches):
            ...
            try:
                # Batch calculation with exclude_special_tokens=True
                batch_lls = compute_batch_likelihood(
                    model, tokenizer, batch_sequences, device, reduce_method,
                    exclude_special_tokens=True,  # ALWAYS exclude 5/3/<eos>
                    batch_size=len(batch_sequences)
                )
                log_likelihoods.extend(batch_lls)
            except Exception as e:
                import traceback
                print(f"[WARNING] Batch {batch_idx} calculation failed: {e}", file=sys.stderr)
                traceback.print_exc(file=sys.stderr)
                # Append NaN for failed sequences
                log_likelihoods.extend([float('nan')] * len(batch_sequences))
