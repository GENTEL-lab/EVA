...
class Attention(nn.Module):
    ...
    def __init__(self, config: RNAGenConfig, layer_idx: int):
        super().__init__()
        self.config = config
    ...
    def prepare_qkv(
        ...
        # KV cache 处理：始终更新 cache（不管是否为第一次调用）
        if use_cache and past_key_value is not None:
            # 转换为 [bsz, num_heads, seq, dim] 格式以便更新 cache
            key_states = key_states.transpose(1, 2)
            val_states = val_states.transpose(1, 2)
            # DynamicCache.update 会将新的 key/value 追加到已有 cache 并返回完整序列
            key_states, val_states = past_key_value.update(key_states, val_states, self.layer_idx)
            # 转换回 [bsz, seq, num_heads, dim] 格式
            key_states = key_states.transpose(1, 2)
            val_states = val_states.transpose(1, 2)
    ...
    def _sdpa_attn(
        ...
        # 尝试使用优化的attention kernel，如果失败则回退到手动实现
        try:
            # 首先尝试Flash Attention（最快）
            ...
            # Flash Attention失败，尝试不指定backend（让PyTorch自动选择）
            try:
                attn_output = F.scaled_dot_product_attention(
                    query_states,
                    key_states,
                ...
                # 所有优化kernel都失败，使用手动实现
                # 手动计算attention: softmax(Q @ K^T / sqrt(d)) @ V
                attn_weights = torch.matmul(query_states, key_states.transpose(2, 3)) / (self.head_dim ** 0.5)
                
                # 应用causal mask
                if causal_mask is not None:
                    attn_weights = attn_weights + causal_mask
                elif k_len == q_len:  # 使用is_causal
