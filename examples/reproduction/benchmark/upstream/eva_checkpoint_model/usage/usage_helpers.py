...
def load_model(
    ...
    # 先加载权重以检查实际的vocab_size
    weights_file = checkpoint_path / "model_weights.pt"
    state_dict = torch.load(weights_file, map_location="cpu")
    
    # 从权重中获取实际的vocab_size和数据类型
    if 'model.embed_tokens.weight' in state_dict:
        actual_vocab_size = state_dict['model.embed_tokens.weight'].shape[0]
        if actual_vocab_size != config.vocab_size:
            print(f"调整 vocab_size: {config.vocab_size} -> {actual_vocab_size}")
            config.vocab_size = actual_vocab_size
    
    # 检测权重的数据类型
    weight_dtype = state_dict['model.embed_tokens.weight'].dtype
    ...
    # 将整个模型转换为 bfloat16（确保内部 MegaBlocks 等组件一致）
    # 参考 optimize_batch.py 的做法
    print("将模型转换为 BFloat16（确保所有内部组件一致）")
    ...
    print(f"  模型词汇表大小: {config.vocab_size}")
...
def build_conditional_prefix(
    ...
    if rna_type:
        ...
        prefix_parts.append(rna_type)

    # 组合前缀
    if prefix_parts:
        # 如果同时有谱系和RNA类型，用分号连接
        if lineage and rna_type:
            prefix = prefix_parts[0] + ';' + prefix_parts[1] + '|'
        else:
            prefix = ''.join(prefix_parts)
            # 如果只有谱系信息，确保以|结尾
            if lineage and not prefix.endswith('|'):
                prefix += '|'
        return prefix
