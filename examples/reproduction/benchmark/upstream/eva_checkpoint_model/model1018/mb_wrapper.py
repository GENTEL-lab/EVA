...
class RNAMoEWrapper(nn.Module):
    ...
    def _setup_megablocks_args(self, **kwargs) -> megablocks.layers.arguments.Arguments:
        ...
        args_dict = {
            ...
            "device": "cuda",
            "mlp_type": "glu" if getattr(self.config, 'gated_mlp', False) else "mlp",
            "grouped_mlp": True,  # 使用grouped模式避免blocking=128限制
            # "mlp_impl": "grouped" if getattr(self.config, 'moe_grouped_gemm', False) else "sparse",  # rnagen容器暂不支持
            "memory_optimized_mlp": getattr(self.config, 'moe_memory_optimized', False),
