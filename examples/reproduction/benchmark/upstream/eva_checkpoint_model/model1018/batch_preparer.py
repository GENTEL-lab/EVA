...
from model1018.token_config import END_OF_SPAN_TOKEN
...
class RNAGenBatchPreparer:
    ...
    def __init__(
        ...
        tokenizer: Optional[LineageRNATokenizer] = None,
    ):
        """初始化批处理准备器
        ...
        Args:
            ...
            rng: 随机数生成器
            tokenizer: LineageRNATokenizer 实例（可选，如不提供则创建新实例）
        """
        self.data_prep_config = data_prep_config or DataPrepConfig()
        self.rng = rng or np.random.default_rng(0)
        
        # 使用 LineageRNATokenizer
        if tokenizer is None:
            self.tokenizer = LineageRNATokenizer()
        else:
