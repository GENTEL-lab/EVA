...
[`reproduction/benchmark_release/`](../reproduction/benchmark_release/).
That notebook performs CPU analysis; fresh model inference has its own command.

[`reproduction/`](../reproduction/README.md) holds fixed inputs, protocols,
reference outputs and dated records, together with preserved historical code.
Some data and helper scripts remain next to their notebooks to keep existing
paths working. Both directories participate in paper reproduction.
