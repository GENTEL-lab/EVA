...
def compute_batch_likelihood(
    ...
    Args:
        ...
        device: Device
        reduce_method: Reduction method ('mean' or 'sum')
        exclude_special_tokens: Whether to exclude 5/3/<eos> token log-likelihood (default: True)
        batch_size: Number of sequences to process at once
        sequence_only: Only compute log-likelihood on pure sequence tokens,
                       excluding <bos>, prefix/tag, 5, 3, <eos> (default: False)
    ...
    # Get 5/3 token IDs for sequence_only mode
    five_token_id = None
    three_token_id = None
    if sequence_only:
        five_ids = tokenizer.encode('5')
        three_ids = tokenizer.encode('3')
        if five_ids:
            five_token_id = five_ids[0]
        if three_ids:
            three_token_id = three_ids[0]

    # Process sequences in batches
    results = []
    num_batches = (len(sequences) + batch_size - 1) // batch_size
    print(f"[BATCH_PROGRESS] Total batches: {num_batches}, batch_size: {batch_size}, total sequences: {len(sequences)}")
    ...
    for batch_idx in range(num_batches):
        ...
        print(f"[BATCH_PROGRESS] Processing batch {batch_idx + 1}/{num_batches} (sequences {start_idx + 1}-{end_idx})")
        ...
        # Padding to maximum length in this batch
        ...
        # Calculate log-likelihood for each sequence in batch
        for i in range(len(batch_sequences)):
            seq_len = seq_lengths[i]

            # Check for nan/inf
            if torch.isnan(logits[i]).any() or torch.isinf(logits[i]).any():
                print(f"[WARNING] Sequence {start_idx + i} logits contain nan or inf values", file=sys.stderr)
            ...
            # In sequence_only mode, find the range of pure sequence tokens
            seq_start = 0  # inclusive: first position where predicted token is a sequence token
            seq_end = seq_len  # exclusive
            if sequence_only and five_token_id is not None and three_token_id is not None:
                input_ids_list = batch_input_ids[i, :seq_len].tolist()
                five_pos = None
                three_pos = None
                for pos, tid in enumerate(input_ids_list):
                    if tid == five_token_id and five_pos is None:
                        five_pos = pos
                    elif tid == three_token_id and five_pos is not None:
                        three_pos = pos
                        break
                if five_pos is not None and three_pos is not None:
                    # Only include predicted tokens at positions (five_pos+1) to (three_pos-1)
                    seq_start = five_pos  # j = five_pos means predicted token at five_pos+1
                    seq_end = three_pos - 1  # j = three_pos-2 means predicted token at three_pos-1

            for j in range(seq_len - 1):
                predicted_token = batch_input_ids[i, j + 1]

                if sequence_only:
                    # Only include tokens in the pure sequence range
                    if j < seq_start or j > seq_end - 1:
                        continue
                    # Also skip special tokens within the range (safety check)
                    if predicted_token.item() in special_token_ids:
                        continue
                elif exclude_special_tokens and predicted_token.item() in special_token_ids:
                    # Original behavior: exclude special tokens
                    continue
        ...
        # Print progress
        if num_batches > 1:
            print(f"Batch {batch_idx + 1}/{num_batches} completed ({end_idx}/{len(sequences)} sequences)", file=sys.stderr)
