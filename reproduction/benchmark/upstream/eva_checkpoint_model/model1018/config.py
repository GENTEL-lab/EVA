...
class RNAGenConfig(PretrainedConfig):
    ...
    def __init__(  # type: ignore
        ...
        **kwargs,
        ...
        # 如果提供了 tokenizer，从中获取 token IDs
        # 否则使用传入的参数值（从 JSON 加载时会提供这些值）
        ...
        # 设置 vocab_size：优先使用 tokenizer，其次使用传入的参数
        if tokenizer is not None:
            tokenizer_vocab_size = tokenizer.vocab_size
            # 如果同时提供了 vocab_size 参数，进行比较和警告
            ...
            # 从 JSON 加载时应该有 vocab_size，如果都没有则设为 None（会在后续使用时报错）
            self.vocab_size = None
        
        self.gradient_checkpointing = gradient_checkpointing
        self.no_ffn_gradient_checkpointing = no_ffn_gradient_checkpointing
        
        # 仅当提供了 tokenizer 时，才检查 token ID 是否与传入参数冲突
        # （从 JSON 加载时不会有 tokenizer，这些值会直接从配置文件中加载）