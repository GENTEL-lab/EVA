...
| Precise checkpoint and dataset identities | `examples/reproduction/milena_14b/manifest.json`, [resources](RESOURCES.md), [comparison resources](OFFICIAL_MODEL_RESOURCES.md) | Representative versions/hashes fixed. Other missing versions remain explicit; formal release and code DOI pending. |
...
| One end-to-end benchmark | [Download → inference → metrics → plots](REPRODUCTION.md#gpu-run-the-complete-milena-benchmark), [expected output](../examples/reproduction/milena_14b/expected/README.md) | Completed for 135 Milena samples. The measured 0.00338 difference is documented and accepted for this example; the reference is preserved. |
