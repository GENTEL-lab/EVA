...
if [[ -z "$CONFIG_FILE" ]]; then
    case "$EXPERIMENT_NAME" in
        '') CONFIG_FILE=finetune/aptamer/script/experiment_config_template.yaml;;
...
docker exec "$CONTAINER_NAME" test -f "$CONTAINER_WORKDIR/finetune/train_finetune.py"
cmd=(docker exec -w "$CONTAINER_WORKDIR" -e "CUDA_VISIBLE_DEVICES=$GPU_ID" "$CONTAINER_NAME" python finetune/train_finetune.py --config "$CONFIG_FILE")
